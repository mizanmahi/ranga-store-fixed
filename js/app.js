...
const showProducts = (products) => {
   ...
   for (const product of allProducts) {
      const image = product.image; // ! here will be product.image
...
const addToCart = (id, price) => {
   ...
   updateTaxAndCharge();
   updateTotal(); //  ! need to call here to update the total
...
const getInputValue = (id) => {
   ...
   const converted = parseFloat(element); // ! here will be parseFloat(element) not the parseInt(element)
...
const updatePrice = (id, value) => {
   ...
   document.getElementById(id).innerText = total.toFixed(2); // ! toFixed(2) is used to show 2 decimal places
};

// set innerText function
const setInnerText = (id, value) => {
   //  document.getElementById(id).innerText = Math.round(value);
   document.getElementById(id).innerText = value; // ! here will be only value to avoid rounding
};
